...
int main(int argc,char* argv[])
{
	...
	if(argc==2){
		...
		strcpy(str,argv[1]);
		strcat(str,"2c19.p2m2fp3.cc.predict");
		data.open(str);

		strcpy(str,argv[1]);
		strcat(str,"2c19.p2m2fp3.cc.rules");
		model.open(str);

		strcpy(str,argv[1]);
		strcat(str,"2c19.p2m2fp3.cc.statistics");
		output.open(str);

		strcpy(str,argv[1]);
		strcat(str,"2c19.p2m2fp3.cc.cases");
		descriptorfile.open(str);
	}
	else{
		data.open("2c19.p2m2fp3.cc.predict");
		model.open("2c19.p2m2fp3.cc.rules");
		output.open("2c19.p2m2fp3.cc.statistics");
		descriptorfile.open("2c19.p2m2fp3.cc.cases");
	}
	...
	//-----read each model
	for( i = 0 ; i < 3; i++ ){// for each model, 癘眔эmodel计ヘ
	...
	while(std::getline(data,line)) // for each compound
    {
		...
		//get model rules
		Fingerprint fp[6930]; //癘眔эΘdescriptor 计ヘx2
		for( i = 0 ; i < 6930; i++){  //癘眔эΘdescriptor 计ヘx2
			fp[i].counter = 0;
			fp[i].index = -1;
			fp[i].condition = -1;
		}
		while(lineStream>>cell){
			...
			for( i = 0; i < ourmodel[modelNumber].GetNumbersOfRule(ruleSetNumber); i++){
				int descriptorType;
				string temp_type1 = (ourmodel[modelNumber].GetDescriptor(ruleSetNumber,i)).substr(0,1);
				string temp_type2 = (ourmodel[modelNumber].GetDescriptor(ruleSetNumber,i)).substr(0,2);
				//// 0->p2  1->m2   2->f   3->p3
				if( temp_type1.compare("P") == 0 && temp_type2.compare("P2") == 0){
					descriptorType = 0;
				}else if( temp_type1.compare("P") == 0 && temp_type2.compare("P3") == 0){
					descriptorType = 3;
				}else if( temp_type1.compare("D") == 0 ){
					descriptorType = 1;
				}else if( temp_type1.compare("F") == 0 ){
					descriptorType = 2;
				}
				int descriptorNumber;
				if( descriptorType == 0 || descriptorType == 3 )
					descriptorNumber = atoi((ourmodel[modelNumber].GetDescriptor(ruleSetNumber,i)).substr(2).c_str());  // p2 or p3
				else
					descriptorNumber = atoi((ourmodel[modelNumber].GetDescriptor(ruleSetNumber,i)).substr(1).c_str());  //f or m2
				...
				fp[descriptorNumber+condition].type = descriptorType;
			}
		}
		qsort((void *)(fp), (size_t)(6930), sizeof(Fingerprint), compareTwoFingerprint);  //癘眔эΘdescriptor 计ヘx2
		...
		for( i = 0 ; i < 5; i++){  //癘眔эformat
			if( fp[i].counter > 0 ){
				if( fp[i].type == 1 ) // m2
					output << "D" << setw(3) << setfill('0') << fp[i].index << "," << fp[i].condition << "," << fp[i].cut <<"," << descriptorvalue[compound_index][fp[i].index+1375];
				else if( fp[i].type == 2) // f
					output << "F" << setw(3) << setfill('0') << fp[i].index << "," << fp[i].condition << "," << fp[i].cut <<"," << descriptorvalue[compound_index][fp[i].index+2153];
				else if( fp[i].type == 0 ) // p2
					output << "P2" << setw(4) << setfill('0') << fp[i].index << "," << fp[i].condition << "," << fp[i].cut <<"," << descriptorvalue[compound_index][map[fp[i].index]];
				else if( fp[i].type == 3 ) // p3
					output << "P3" << setw(3) << setfill('0') << fp[i].index << "," << fp[i].condition << "," << fp[i].cut <<"," << descriptorvalue[compound_index][fp[i].index+3034];
				temp_number++;
			}
